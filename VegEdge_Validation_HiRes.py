...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  9 16:24:12 2022
...
#%%
metadata = {'Earthi':None}
metadata['Earthi'] = {'filenames':['Earthi_TRIPLESAT_20180625100239'],'epsg':[27700],'acc_georef':[1.144], 'dates':['2018-06-25']}
...
for i,j,c in zip(range(7),[0,1,2,4,8,19,30],['#FCFFA1','#FBB314','#ED641F','#BA3251','#75176A','#2F0B5B','#07070A']):
    veg_x = list([])
    sat_y = list([])
...
plt.ylim((0,600))
plt.legend()
plt.savefig('/media/14TB_RAID_Array/User_Homes/Freya_Muir/PhD/Year2/Outputs/Figures/VegSat_StAndrews_Errors_MEdenside.png')
plt.show()
...
for i,j,c in zip(range(7),[0,1,2,7,9,15,18],['#FCFFA1','#FBB314','#ED641F','#BA3251','#75176A','#2F0B5B','#07070A']):
    veg_x = []
    sat_y = []
...
plt.plot(output_proj_West['dates'],output_proj_West['Otsu_threshold'], 'o', color=colors[0],  label='West/Inner estuarine')
plt.plot(output_proj_East['dates'],output_proj_East['Otsu_threshold'], 'o', color=colors[1], label='East/Open coast')
