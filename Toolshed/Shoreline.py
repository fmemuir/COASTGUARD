...
def extract_shorelines(metadata, settings, polygon, dates):
    """
    ...
    for satname in metadata.keys():
        ...
        for i in range(len(filenames)):
            ...
            if settings['adjust_detection']:
                ...
                skip_image, shoreline, shoreline_latlon, shoreline_proj, t_mndwi = adjust_detection(im_ms, cloud_mask, im_labels,
            ...
            # otherwise use find_wl_contours2 (traditional)
                ...
                try: # use try/except structure for long runs
                    if sum(sum(im_labels[:,:,0])) < 10 : # minimum number of sand pixels
                        ...
                        contours_mwi, t_mndwi = find_wl_contours2(im_ms, im_labels, cloud_mask,buffer_size_pixels, im_ref_buffer)
                except:
                    print('Could not map shoreline for this image: ' + filenames[i])
                    continue
    
                # process the water contours into a shoreline
                shoreline, shoreline_latlon, shoreline_proj = process_shoreline(contours_mwi, cloud_mask, georef, image_epsg, settings)
...
def find_wl_contours2(im_ms, im_labels, cloud_mask, buffer_size, im_ref_buffer):
    ...
    im_mwi = Toolbox.nd_index(im_ms[:,:,4], im_ms[:,:,1], cloud_mask)
    # calculate Normalized Difference Modified Water Index (NIR - G)
    im_wi = Toolbox.nd_index(im_ms[:,:,3], im_ms[:,:,1], cloud_mask)
...
def process_shoreline(contours, cloud_mask, georef, image_epsg, settings):
    ...
    contour_proj = Toolbox.convert_epsg(contours_world, image_epsg, settings['projection_epsg'])
...
def adjust_detection(im_ms, cloud_mask, im_labels, im_ref_buffer, image_epsg, georef,
    ...
    # process the water contours into a shoreline
    shoreline, shoreline_latlon, shoreline_proj = process_shoreline(contours_mndwi, cloud_mask, georef, image_epsg, settings)
    ...
    return skip_image, shoreline, shoreline_latlon, shoreline_proj, t_mndwi