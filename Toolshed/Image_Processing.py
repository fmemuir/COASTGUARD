...
def create_cloud_mask(im_QA, satname, cloud_mask_issue):
    ...
    if sum(sum(cloud_mask)) > 0 and sum(sum(~cloud_mask)) > 0:
        morphology.remove_small_objects(cloud_mask, min_size=10, connectivity=1) # removed in_place=True (CM)
        ...
        if cloud_mask_issue:
            ...
            morphology.remove_small_objects(cloud_mask, min_size=25, connectivity=1) # removed in_place=True (CM)
