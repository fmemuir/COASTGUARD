...
def PCABiplot(MultivarGDF):
    
    # Classify plot based on eroding vs accreting veg
    MultivarGDF.columns = ['VegEdgeRate', 'WaterlineRate', 'TransitionZoneWidth','MaxSlopeAtVE','WaveDiffus']
    MultivarGDF['Class'] = 1
    MultivarGDF['Class'].iloc[:int(len(MultivarGDF)/2)] = 0
    # MultivarGDF.reset_index(inplace=True)

    # Separate features and labels
    features = MultivarGDF.drop('Class', axis=1)
    labels = MultivarGDF['Class']
    ...
    d = {'color': ['#C51B2F','#5499DE']}
    fg = sns.FacetGrid(data=principal_df, hue='Class', hue_kws=d, height=8)
    fg.map(plt.scatter, 'PC1', 'PC2').add_legend()
    # Add arrows for the original variables
    for i, (component, feature) in enumerate(zip(pca.components_.T, features.columns)):
        plt.arrow(0, 0, component[0], component[1], color='r', alpha=0.5, width=0.01)
        plt.text(component[0] * 1.15, component[1] * 1.15, feature, color='g', ha='center', va='center', fontsize=12)

    plt.xlabel(f'Principal Component 1')
    plt.ylabel(f'Principal Component 2')
    plt.title('PCA Biplot with Predefined Clusters')
    plt.grid()
    plt.show()
    ...
    # # Perform PCA
    # principal_components = pca.fit_transform(MultivarGDFStd)
    # principal_df = pd.DataFrame(data=principal_components, columns=[f'PC{i+1}' for i in range(len(pca.components_))])
    ...
    # # Create the biplot
    # def biplot(pca, pc1=0, pc2=1, labels=None):
    #     plt.figure(figsize=(10, 8))
    #     sns.scatterplot(x=principal_df.iloc[:, pc1], y=principal_df.iloc[:, pc2], hue=principal_df['Cluster'], palette='Set1', s=100, alpha=0.6)
        
    #     for i, (component, feature) in enumerate(zip(pca.components_, MultivarGDF.columns)):
    #         plt.arrow(0, 0, component[pc1], component[pc2], color='r', alpha=0.5, width=0.005)
    #         plt.text(component[pc1] * 1.15, component[pc2] * 1.15, feature, color='g', ha='center', va='center', fontsize=12)
    
    #     plt.xlabel(f'Principal Component {pc1+1} [expl. var. = {round(Var[0]*100,2)}%]')
    #     plt.ylabel(f'Principal Component {pc2+1} [expl. var. = {round(Var[1]*100,2)}%]')
    #     plt.title(f'Biplot [{CompNum} components explain {round(np.sum(Var[:CompNum]),2)*100}% of variance]')
    #     plt.grid()
